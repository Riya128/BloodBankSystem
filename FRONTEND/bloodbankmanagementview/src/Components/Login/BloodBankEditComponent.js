...
function BloodBankEditComponent() {
  let token = JSON.parse(localStorage.getItem("bloodbanktoken"));
  let id = JSON.parse(localStorage.getItem("bloodbankid"));
  const base_url = "http://localhost:8999/bloodbank-service/bloodbank";
  let URL = `${base_url}/${id}`;
  ...
  const updateDataToServer = (data, e) => {
    axios
      ...
      .then(
        (response) => {
          alert("BloodBank Details Updated Successfully");
        },
        (error) => {
  ...
  return (
    <div>
      ...
      <header>
        <nav className="navbar navbar-expand-md navbar-dark bg-dark">
          ...
          <div className="button">
            <a
              style={{ marginLeft: "30px", marginRight: "950px" }}
              href="/editBloodBank"
      ...
      <h2 style={{ textAlign: "center" }}>Update BloodBank Details</h2>
      <div style={{ marginRight: "30%", marginBottom: "40%" }}>
        <form onSubmit={(e) => FormHandle(e)} style={{ width: "40%" }}>
          ...
          <div>
            <button
              type="submit"
              class="btn btn-outline-success my-2 text-center mr-2"
              style={{ marginLeft: "40%" }}
            >
              Update{" "}
            </button>
...
export default BloodBankEditComponent;
